...
def _find_opportunity_windows(forecast_items: list[dict]) -> list[dict]:
    ...
    Returns list of dicts with window_start, window_end, estimated_kwh, reward_points.
    """
    export_hours = []
    for item in forecast_items:
        ...
        # Skip waking-hours filter: midnight to 05:00
        if dt_val.hour < 5:
            continue
...
async def notify_flexibility_opportunity(ctx: EventContext) -> None:
    """Notify community participants about flexibility opportunities.

    Triggered when the rec-forecasting-flow pipeline completes. Fetches the
    24h REC forecast, detects net-export windows (excess solar), and sends a
    nudging_flexibility_opportunity event to each community participant.
    """
    now = datetime.now(timezone.utc)
    end = now + timedelta(hours=24)

    try:
        forecast_response = await ctx.infra.values_service.fetch(
            fetcher_id="it-energy-community.rec_forecast",
            payload={
                "start": now.isoformat(),
                "end": end.isoformat(),
            },
    ...
    # Use the first (earliest) window for the notification
    ...
    # Fetch all community participants
    try:
        rec_registry_admin: RecRegistryAdminClient = ctx.infra.clients_registry.get(
            "rec_registry_admin"
        )
        members = await rec_registry_admin.list_members()
    except Exception as exc:
    ...
    for member in members:
        ...
        try:
            ...
            logger.debug(
                "Sent flexibility_opportunity nudge to user=%s community=%s window=%s-%s",
                user_id,
                community_id,
                window_start_str,
                window_end_str,
            ...
            logger.warning(
                "Failed to send flexibility nudge to user=%s: %s", user_id, exc
            )
