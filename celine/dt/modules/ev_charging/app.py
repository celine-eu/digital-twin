...
from celine.dt.modules.ev_charging.datasets import (
    build_dwd_solar_query,
    build_weather_hourly_query,
)
...
class EVChargingReadinessApp(
    ...
    async def run(
        ...
        sql_query = build_dwd_solar_query(
        ...
        solar_total = 0.0
        if dwd_rows:
            # initial heuristic: cumulative series → take max as total up to window end
            solar_total = float(
        ...
        # Confidence: stable clouds => higher confidence; volatility penalizes.
        # std=0  -> 1.0
        # std>=50 -> 0.0
        confidence = max(0.0, min(1.0, 1.0 - (std_clouds / 50.0)))

        # --- 3) Community PV energy projection -----------------------------------
        #
        # We intentionally keep this explainable and avoid PV physics details.
        # A simple, defensible proxy:
        #   expected_pv_kwh ≈ pv_capacity_kw × (solar_total_kwh_per_m2) × (1 - mean_clouds_pct)
        #
        # Interpretation:
        # - solar_total_kwh_per_m2 captures daylight + seasonality from DWD
        # - (1 - clouds) dampens yield under cloud cover
        #
        # This is *not* a replacement for production models—it's a decision proxy.
